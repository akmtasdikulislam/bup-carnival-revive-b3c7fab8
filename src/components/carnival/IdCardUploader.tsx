...
async function getCroppedImg(src: string, pixels: Area, rotation: number): Promise<string> {
  ...
  // First, rotate the image onto a large canvas, then crop from it.
  ...
  ctx.drawImage(
    rot,
    safeSize / 2 - image.width / 2 + pixels.x,
    safeSize / 2 - image.height / 2 + pixels.y,
    pixels.width,
    pixels.height,
    0,
    0,
